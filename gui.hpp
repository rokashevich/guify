...
class Gui {
 ...
 public:
  ...
  //    // Ширину окна рассчитваем, чтобы влезали названия панелей и их
  //    содержимое.
  //    // А высота окна чтобы была в 75% от доступной. В дальнейшем высота
  //    будет
  ...
  //      } else if (key == "-D") {
  //        panel =
  //            new DirectoryChooser(cumulative_height, panel_name_width,
  //            window_w,
  ...
  //          static_cast<std::vector<Panel*>*>(x); std::vector<std::string>
  //          results; for (Panel* a : *panels) {
  ...
  //    Fl_Button* cancel = new Fl_Button(ok_cancel_width, window_h -
  //    panel_height,
  //                                      ok_cancel_width, panel_height,
  //                                      "Cancel");
  ...
  Gui(Cfg* cfg) {
    ...
    //    Fl::screen_work_area(work_area_x_, work_area_y_, work_area_w_,
    //    work_area_h_,
    //                         0);
